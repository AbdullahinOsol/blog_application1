...
class User(UserMixin, db.Model):
    ...
    following: so.WriteOnlyMapped['User'] = so.relationship(secondary=followers, primaryjoin=(followers.c.follower_id == id),
                                                            secondaryjoin=(followers.c.followed_id == id), back_populates='followers', passive_deletes=True)
    
    followers: so.WriteOnlyMapped['User'] = so.relationship(secondary=followers, primaryjoin=(followers.c.followed_id == id),
                                                            secondaryjoin=(followers.c.follower_id == id), back_populates='following', passive_deletes=True)
...
class Post(db.Model):
    id: so.Mapped[int] = so.mapped_column(primary_key=True)
    title: so.Mapped[str] = so.mapped_column(primary_key=True)
    body: so.Mapped[str] = so.mapped_column(sa.String(750))
    ...
    comments: so.WriteOnlyMapped['Comment'] = so.relationship('Comment', back_populates='post', cascade='all, delete-orphan', passive_deletes=True)

    def __repr__(self):
        return '<Post {}>'.format(self.body)
    
    def get_comments(self):
        query = sa.select(Comment).where(Comment.post_id == self.id).order_by(Comment.timestamp.desc())
        result = db.session.execute(query)
        return result.scalars().all()
    
class Comment(db.Model):
    id: so.Mapped[int] = so.mapped_column(primary_key=True)
    body: so.Mapped[str] = so.mapped_column(sa.String(70))
    timestamp: so.Mapped[datetime] = so.mapped_column(
        index=True, default=lambda: datetime.now(timezone.utc))
